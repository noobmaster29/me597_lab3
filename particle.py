...
class particle:
    ...
    def motion_model(self,dth, v, w, dt):
        self.pose[0] = self.pose[0] + v * dt * cos(self.pose[2] + dth/2)
        self.pose[1] = self.pose[1] + v * dt * sin(self.pose[2] + dth/2)
    ...
    #Reasons to use log
        #Numerical stability 
            #Direct likelihood from sensor measurements can results in extremely small numbers
            #This can lead to numerical errors
        #Computation efficiency
            #It is easier to to apply optimization algorithms on the sum of logs
