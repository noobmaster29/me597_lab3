...
class particleFilter(Node):
    ...
    def normalizeWeights(self):
        ...
        self.weights = np.array(self.weights) * self.normalizerFactor #Modified
        #self.weights = self.weights * self.normalizerFactor #Modified
    ...
    def resample(self, laser_scan, mapUtilInstance):
        ...
        best_particles = particles_sorted[:len(self.particles) // 2]
        ...
        std_noise=0.1 #Adjust as needed
        ...
        for bp in best_particles:
            ...
            new_particle = particle([new_x, new_y, new_th], 1/self.numParticles)
    ...
    def filterCallback(self, odomMsg: Odometry, laserMsg: LaserScan):
        ...
        # Calculate the mean angle using the mean of circular quantities
        angles = self.particlePoses[:, 2]
        sin_sum = np.sum(np.sin(angles) * self.weights)
        cos_sum = np.sum(np.cos(angles) * self.weights)
        ...
        self.championPose = weighted_average_pose
